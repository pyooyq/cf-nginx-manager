...
本机 Nginx 服务地址必须填写为 `http://HOST:PORT`，例如 `http://127.0.0.1:8080`，不要使用 `https://`，也不要带路径、查询参数或片段。`http://` 前缀可以省略，直接填 `HOST:PORT`（如 `127.0.0.1:8080`）会自动补全。

它只在「Tunnel + Nginx 反代」（`proxy`/`mirror`/`cfcdn`）模式下使用：Tunnel 先把流量交到本机这个地址上的 Nginx，再由 Nginx 反代到目标站。`direct`（Tunnel 直连）和 `public`（公网入站）模式不使用它。首次配置时如果默认的 `8080` 端口已被占用，脚本会自动建议一个空闲端口；需要修改时运行 `cfp config`（主菜单 5）。已有 proxy/mirror/cfcdn 站点时修改端口，脚本会自动重新生成这些站点的 Nginx 配置并同步 Tunnel ingress。
