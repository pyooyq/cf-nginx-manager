...
它只在「Tunnel + Nginx 反代」（`proxy`/`mirror`/`cfcdn`）模式下使用：Tunnel 先把流量交到本机这个地址上的 Nginx，再由 Nginx 反代到目标站。`direct`（Tunnel 直连）和 `public`（公网入站）模式不使用它。首次配置时如果默认的 `8080` 端口已被占用，脚本会自动建议一个空闲端口；需要修改时运行 `cfp config`（主菜单 5）。注意：已有反代站点时修改端口，需要重新编辑那些站点（重新生成 Nginx 配置）并执行 `cfp sync`。
